...
directly, so you can confirm your token and Outreach Status property work
against your real HubSpot account without spending any Anthropic API
budget or waiting on a real lead.
...
Usage:
    ...
    python test_hubspot_connection.py YOUR_HUBSPOT_TOKEN [outreach_property]
...
try:
    ...
    print('Go check HubSpot for a Company called "BD Agent Test Co" — its Outreach Status should say "Contacted".')
except hubspot_sync.HubSpotAPIError as exc:
    print(f"FAILED: {exc}")
    print("\nIf the error mentions the property not existing, double-check the internal name — "
          "it needs to be the Company object's internal name specifically.")
    sys.exit(1)
